...
export default function UiuxProjectDetailPage() {
  const { slug } = useParams();
  const navigate = useNavigate();
  const item = slug ? getProjectBySlug(slug) : undefined;
  ...
  return (
    <div className="min-h-screen bg-background">
      <SEO
        title={`${item.meta.title} | UI/UX Project`}
        ...
        <main className="container mx-auto px-6 py-10 max-w-5xl space-y-8">
          ...
          <Card>
            <CardContent className="py-6 space-y-6">
              ...
              <ShareButton title={item.meta.title} text={item.meta.summary} shareType="project" slug={`uiux/${item.slug}`} />
            </CardContent>
          </Card>
        </main>
      </TooltipProvider>
